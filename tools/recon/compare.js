...
(function rides(){
  const db = load("rides");
  const lines = ["# Rides discrepancies","","| Day | Status | DB rides | DB total_fare | API note | Δ |","|---|---:|---:|---:|---|---:|"];
  for (const r of db){
    lines.push(`| ${r.day} | ${r.status} | ${r.rides} | ${r.total_fare} | (check admin /rides/summary) | — |`);
  }
  write("rides", lines);
...
(function tips(){
  const db = load("tips");
  const lines = ["# Tips discrepancies","","| Day | DB tips | DB tip_amount | API note |","|---|---:|---:|---|"];
  for (const r of db){ 
    lines.push(`| ${r.day} | ${r.tips} | ${r.tip_amount} | (check admin /tips/summary) |`); 
  }
  write("tips", lines); 
...
(function payouts(){
  ...
  const lines = ["# Payouts discrepancies","","| Day | Status | DB requests | DB amount | API note |","|---|---|---:|---:|---|"];
  for (const r of db){ 
    lines.push(`| ${r.day} | ${r.status} | ${r.requests} | ${r.amount} | (check admin /payouts/summary) |`); 
  }
  write("payouts", lines); 
...
(function referrals(){
  ...
  const lines = ["# Referrals discrepancies","","| Day | Tier | DB registrations | DB reward_amount | API note |","|---|---|---:|---:|---|"];
  for (const r of db){ 
    lines.push(`| ${r.day} | ${r.tier} | ${r.registrations} | ${r.reward_amount} | (check admin /referrals/summary) |`); 
  }
...
(function invoices(){
  ...
  const lines = ["# Invoices discrepancies","","| Month | Status | DB invoices | DB total_amount | API note |","|---|---|---:|---:|---|"];
  for (const r of db){ 
    lines.push(`| ${r.month} | ${r.status} | ${r.invoices} | ${r.total_amount} | (check admin /invoices/summary) |`); 
  }
